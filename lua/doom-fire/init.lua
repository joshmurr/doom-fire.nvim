...
local function draw(buffer, w, h)
	local lines = {}
	local hl_colors = {}
	for y = 0, h do
		local row = ""
		for x = 0, w do
			...
			M.pix_buf[idx - decay] = math.max(M.pix_buf[below] - decay, 0)
			hl_colors[idx] = "Doom" .. M.pix_buf[idx - decay] + 1

			if M.options.show_color_vals then
				row = row .. string.format("%02d", M.pix_buf[idx - decay])
			else
				row = row .. "  "
			end
		end
